...
pub trait DBConnection: Send + Sync {
    ...
    async fn create_user(&self, email: &str, hash: &str,
                         is_admin: bool, is_confirmed: bool) -> Result<(), Error>;
...
impl<T: DBConnection> DBConnection for std::sync::Arc<T> {
    ...
    async fn create_user(&self, email: &str, hash: &str,
                         is_admin: bool, is_confirmed: bool) -> Result<(), Error> {
        T::create_user(self, email, hash, is_admin, is_confirmed).await
    }
    async fn update_user(&self, user: &User) -> Result<()> {
...
impl<T: DBConnection> DBConnection for tokio::sync::Mutex<T> {
    ...
    async fn create_user(&self, email: &str, hash: &str,
                         is_admin: bool, is_confirmed: bool) -> Result<(), Error> {
        self.lock().await.create_user(email, hash, is_admin, is_confirmed).await
    }
    async fn update_user(&self, user: &User) -> Result<()> {
